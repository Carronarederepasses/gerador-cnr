...
| **Fase 0 concluída, falta fechar** | Banco e funil prontos (23/set). Falta a segunda migração: tirar o DEFAULT e tornar `conta_id` obrigatório. Storage (arquivos) ainda é separado por pasta do dono, não por regra. |
| **Quando virar a chave do multi-loja** | Hoje cada loja é **um site + um banco** (CNR e piloto). Isso para no terceiro: o Supabase grátis só deixa 2 bancos ativos. O gatilho da virada **não é o segundo cliente pagante** — esperar até ele é fazer a mudança com duas lojas trabalhando ao vivo, e um erro atinge quem paga. O gatilho é o **sinal de que vem a segunda loja**: o piloto usando de verdade mais alguém demonstrando interesse. Aí migra com só o piloto dentro, e o segundo cliente já entra na casa pronta. Antes de virar: `conta_id` obrigatório, storage por conta, e o teste de duas contas no mesmo banco sem vazamento. |
...
| **Multi-tenancy (o resto)** | Isolamento **existe hoje por dois caminhos**, e é preciso não confundi-los. **(1) Entre lojas:** cada loja é um site + um banco (CNR e piloto), e dentro do código o `conta_id` + funil `_db.js` já separam. **(2) Dentro da loja:** as chaves `CNR_KEY_2..9` dizem **quem** operou (o card mostra), e de propósito **não** separam dados — Yuri e mãe são a mesma operação. O que **falta** é multi-loja no **mesmo** banco: `conta_id` obrigatório (hoje tem DEFAULT), storage por regra e não por pasta, e identidade de verdade no lugar da chave por aparelho (fase 1). `seen`/`queue` seguem no storage local de cada máquina. |
